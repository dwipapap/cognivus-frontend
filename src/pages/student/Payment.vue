...
const handlePayment = async () => {
    ...
    try {
        ...
        // Tuition and ancillary fees are generated by separate backend endpoints
        const target = resolvePaymentTarget(selectedPaymentType.value);
        if (!target) {
        ...
        const response = target.kind === "tuition"
            ? await paymentAPI.generateTuitionToken(paymentData)
            : await paymentAPI.generateAncillaryToken(paymentData);
