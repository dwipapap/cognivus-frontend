...
<template>
  ...
  <div v-else-if="course" class="space-y-4 md:space-y-6 mb-8">
    ...
    <section class="bg-white rounded-xl shadow-lg p-4 md:p-6">
      ...
      <div v-if="course.description" class="mb-6 md:mb-8">
        <div class="flex items-center gap-2 md:gap-3 mb-3 md:mb-4">
          <div class="p-1.5 md:p-2 bg-blue-100 rounded-lg text-blue-600">
            <UIcon name="i-basil-document-solid" class="w-5 h-5 md:w-6 md:h-6" />
          </div>
      ...
      <div>
        <div class="flex items-center gap-2 md:gap-3 mb-4 md:mb-6">
          <div class="p-1.5 md:p-2 bg-blue-100 rounded-lg text-blue-600">
            <UIcon name="i-basil-folder-solid" class="w-5 h-5 md:w-6 md:h-6" />
          </div>
        ...
        <div v-if="courseFiles.length === 0 && !course.video_link" class="text-center py-8 md:py-12 text-gray-500">
          <UIcon name="i-basil-folder-solid" class="w-12 h-12 md:w-16 md:h-16 mx-auto mb-2 md:mb-3 text-gray-300" />
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          ...
          >
            ...
            <div class="flex-shrink-0 w-14 h-16 md:w-16 md:h-16 bg-white rounded-lg flex items-center justify-center">
              <div class="text-center">
                <UIcon name="i-basil-document-solid" class="w-7 h-7 md:w-8 md:h-8 mx-auto text-blue-600" />
            ...
            <!-- Open Icon -->
            <UIcon name="i-basil-document-solid" class="w-5 h-5 md:w-6 md:h-6 text-white/80 flex-shrink-0" />
          </button>
          ...
          >
            <!-- Video Thumbnail -->
            <div class="flex-shrink-0 w-14 h-16 md:w-16 md:h-16 bg-white rounded-lg flex items-center justify-center">
              <UIcon name="i-basil-play-solid" class="w-9 h-9 md:w-10 md:h-10 text-red-600" />
            </div>
            ...
            <!-- Open Icon -->
            <UIcon name="i-basil-upload-outline" class="w-5 h-5 md:w-6 md:h-6 text-white/80 flex-shrink-0" />
          </a>
        </div>
      </div>
    </section>
    ...
    <div class="flex justify-end pt-2 md:pt-4">
      <button
        @click="navigateTo({ name: 'StudentMyCourses' })"
      ...
      >
        <UIcon name="i-basil-arrow-left-outline" class="w-4 h-4 md:w-5 md:h-5" />
        <span>Back</span>
      </button>
    </div>
