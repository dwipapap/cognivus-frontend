...
/** Navigate back */
const goBack = () => {
  navigateTo({ name: 'AdminManageStudents' });
};
    ...
    <div v-else-if="student" class="space-y-8">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gray-50/30">
          ...
          <BaseButton
            variant="primary"
            @click="navigateTo({ name: 'AdminAddGrade', params: { userid: studentId } })"
          >
            <template #icon>
              ...
              <tr v-for="grade in grades" :key="grade.gradeid" class="hover:bg-gray-50 transition-colors group">
                ...
                <td class="px-6 py-4 text-right">
                  <BaseButton
                    ...
                    variant="secondary"
                    rounded="lg"
                    @click="navigateTo({ name: 'AdminEditGrade', params: { userid: studentId, gradeid: grade.gradeid } })"
                  >
                    Edit
                  </BaseButton>
                </td>
